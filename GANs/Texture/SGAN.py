...
class Generator(nn.Module):
	...
	# state size. (ngf) x 32 x 32
        nn.ConvTranspose2d( ngf, nc, 5, 1, 0, bias=False),
        nn.Tanh()
...
class Discriminator(nn.Module):
    def __init__(self, ngpu):
        ...
        nn.Conv2d(nc, ndf, 5, 1, 0, bias=False),
...
fixed_noise = torch.randn(64, nz, l, m, device=device)
...
for epoch in range(num_epochs):
    ...
    for i, data in enumerate(dataloader):
        ...
        label = torch.full((b_size,l,m), real_label, device=device)
        ...
        noise = torch.randn(b_size, nz, l, m, device=device)
    ...
    # Check how the generator is doing by saving G's output on fixed_noise
        if (errG.item()<8 and errD.item()>0.2):
            ...
            j+=1   
            if os.path.exists('./outputSGAN/epoch{}'.format(epoch)):
                io.imsave(('./outputSGAN/epoch{}/out{}.png').format(epoch,j,j),np.transpose(vutils.make_grid(fake           	     	   		        [:64],padding=2,                     	        normalize=True),(1,2,0)))
                io.imsave(('./outputSGAN/epoch{}/in{}.png').format(epoch,j),np.transpose(vutils.make_grid(data[0].to(device)[:64],padding=5, 			normalize=True).cpu(),(1,2,0)))
                
            else:
                os.mkdir('./outputSGAN/epoch{}'.format(epoch))
                io.imsave(('./outputSGAN/epoch{}/out{}.png').format(epoch,j,j),np.transpose(vutils.make_grid(fake  	 	 	 	 	[:64],padding=2,                     	        normalize=True),(1,2,0)))			 
                io.imsave(('./outputSGAN/epoch{}/in{}.png').format(epoch,j),np.transpose(vutils.make_grid(data[0].to(device)[:64], padding=5, 			normalize=True).cpu(),(1,2,0))) 
